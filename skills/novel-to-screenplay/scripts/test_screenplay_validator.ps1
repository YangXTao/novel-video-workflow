...
收束：林舟站在桥上，无新伤势说明。
"@
$audit=[ordered]@{schema_version='screenplay-trigger-audit-v2';rule_version=$ruleVersion;rule_sha256=$hash;scenes=@(@{scene_id='SC01';events=@(@{source_quote='林舟从半空摔下，落在桥面';content_type='动作';event='坠落';rule_locator='事件运镜速查表/坠落';screenplay_phrase=$phrase;reason='被动从高处落下，不是主动俯冲；起身不映射权力反转'})});downstream_gate=[ordered]@{status='ready';blocking_ambiguities=@();nonblocking_notes=@()}}
...
foreach($case in $cases){
 $audit.rule_version=if($case.ContainsKey('rule_version')){$case.rule_version}else{$ruleVersion}
...
$audit.rule_version=$ruleVersion
