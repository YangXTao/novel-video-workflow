...
`v10_prompts`完成时只要求登记一个存在、未变化且包含连续S编号的完整章节视频提示词Markdown。默认文件名为`<章节名>_完整视频提示词_v13.0.md`；用户要求候选版本或对比测试时可追加时间戳。历史`shot_production_plan.json`、`scene_shot_map.json`、逐镜文件和`video_prompt_validation.json`可以保留，但不再是阶段完成条件。
